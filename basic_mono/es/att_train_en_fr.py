...
from keras.layers import Dense, Activation, Dropout, Embedding, LSTM, GRU, Bidirectional, Merge, BatchNormalization, merge, Dot, Multiply, Lambda
...
def build_model():
    ...
    x=GRU(3*dim, return_sequences=True)(desc_input)
    x=GRU(3*dim, return_sequences=True)(x)
    x=GRU(3*dim, return_sequences=True)(x)
    x=GRU(3*dim, return_sequences=True)(x)
    x=GRU(3*dim, return_sequences=True)(x)
    x=GRU(2*dim, return_sequences=True)(x)
...
model.fit(train_d, train_w, batch_size=batchsize1, epochs=300)

...
filename=['./es/eswiktionary-latest-pages-articles.xml', './es/eswiktionary-latest-pages-meta-current.xml']
output='./es/en_es_dict.csv'
outputmono = './es/es_dict.csv'
start = 0

max_para = 6

meaning_start = set(['=={{adjetivo|es}}==', '=={{sustantivo masculino|es}}==', '=={{sustantivo femenino|es}}==', '=={{interjeccion|es}}==', '=={{verbo transitivo|es}}==', '=={{infinitivo|es}}==', '=={{verbo transitivo|es}}==', '=={{pronombre|es}}==', '=={{gerundio|es}}==', '=={{participio pasivo|es}}==', '=={{adverbio|es}}==', '=={{preposicion|es}}==', '=={{articulo|es}}==', '=={{conjuncion|es}}==', '=={{adjetivó|es}}==', '=={{sustantivó masculinó|es}}==', '=={{sustantivó femeninó|es}}==', '=={{interjección|es}}==', '=={{verbó transitivó|es}}==', '=={{infinitivó|es}}==', '=={{verbó transitivó|es}}==', '=={{prónómbre|es}}==',   '=={{gerundió|es}}==', '=={{participió pasivó|es}}==', '=={{adverbió|es}}==', '=={{prepósición|es}}==', '=={{articuló|es}}==', '=={{cónjunción|es}}=='])
translate_start = set(['==Traducciónes==','==traducciónes==','==Traducciones==','==traducciones==','==Traducción==','==traducción==','==Traduccion==','==traduccion=='])
...
slang_filt = ['|es', 'es|','=','lan=','par ext|',u'spéc|','part|',u'caténatif|',u'|spéc','|part',u'|caténatif','|en','|en']
        ...
        if (not skip_title) and got_title and (not got_text):
            ...
            for ind in range(len(content)):
                ...
                if role == 0:
                    ...
                    if len(line) < 3:
                        continue
                    if line[0] == ';' and line[2] == ':' and isInt(line[1]):
                        line = desc_start + line[3:]
                    elif line[0] == ';' and line[2] == ' ' and isInt(line[1]):
                        line = desc_start2 + line[3:]
                    ...
                    if len(line) > len(common_end) and line[:len(common_end)] == common_end:
                        ...
                        if not this_lan is None:
                            ...
                            try:
                                this_trans, _, _ = find_between(line, this_lan, '}}')
                                this_trans = this_trans.split('|')[-1]
                            except:
                                continue
